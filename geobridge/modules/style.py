...
Style file generation for QGIS (.qml) and ESRI ArcGIS (.lyrx).
...
def _build_lyrx(preset: dict, n_stops: int = 11) -> str:
    """Construct an ESRI ArcGIS Pro .lyrx JSON document."""
    vmin = float(preset["min"])
    vmax = float(preset["max"])
    label = preset["label"]
    palette = preset["palette"]
    ...
    classes = [
        {
            ...
            "upperBound": value,
            "label": f"{value:g} {preset['unit']}".strip(),
            "color": {
                "type": "CIMRGBColor",
                "values": [r, g, b, 100],
            },
    ...
    document = {
        ...
        "layers": [{
            "type": "CIMRasterLayer",
            "name": label,
            "description": f"Generated by GeoBridge — {label}",
            "colorizer": {
...
def to_esri_lyrx(
    ...
    """
    Generate an ESRI ArcGIS Pro .lyrx file for a Copernicus variable.

    Parameters mirror :func:`to_qgis_style`.

    Examples
    --------
    >>> lyrx = gb.to_esri_lyrx("utci")
    """
    if style_type not in {"raw", "anomaly", "percentile"}:
        raise ValueError(
            f"style_type must be 'raw', 'anomaly', or 'percentile'; "
    ...
    lyrx_json = _build_lyrx(preset)

    if output_path is None:
        suffix = "" if style_type == "raw" else f"_{style_type}"
        output_path = Path.cwd() / f"{variable}{suffix}.lyrx"
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(lyrx_json, encoding="utf-8")

    logger.info("Wrote ESRI style to %s", output_path)
    return output_path
