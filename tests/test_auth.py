"""Tests for geobridge.auth — credential resolution and bearer token API."""
...
def test_authenticate_with_config_file(monkeypatch, tmp_path):
    ...
    config.write_text("url: https://cds.climate.copernicus.eu/api\nkey: file-key-1234\n")
...
def test_authenticate_with_equals_sign_in_config(monkeypatch, tmp_path):
    config = tmp_path / ".cdsapirc"
...
def test_get_token_returns_api_key():
    """The CDS API key is the bearer token under the new ARCO model."""
    auth.authenticate(key="my-api-key")
    assert auth.get_token() == "my-api-key"


def test_auth_header_returns_bearer():
    auth.authenticate(key="my-api-key")
    header = auth.auth_header()
    assert header == {"Authorization": "Bearer my-api-key"}


def test_auth_header_without_authenticate_raises():
